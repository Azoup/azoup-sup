...
import { DEV_NOTES_SYSTEM_EMAIL } from '@/lib/confecKanbanDevNotes';
import { devTicketLabel } from '@/lib/confecKanbanTicketNumber';
import {
...
interface ConfecCardCommentsProps {
  cardId: string;
}
...
async function fetchConfecCardComments(cardId: string): Promise<DevCommentRow[]> {
  const { data, error } = await supabase
    .from('confec_kanban_card_comments')
    .select('*')
    .eq('card_id', cardId)
...
export function ConfecCardComments({ cardId }: ConfecCardCommentsProps) {
  const { user } = useAuth();
  ...
  const { data: comments = [], isLoading, isError, refetch } = useQuery({
    queryKey: ['confec-card-comments', cardId],
    queryFn: () => fetchConfecCardComments(cardId),
    enabled: !!cardId,
    staleTime: 30_000,
    retry: 1,
  });
  ...
  useEffect(() => {
    const channel = supabase
      .channel(`confec-comments-${cardId}`)
      .on('postgres_changes', {
        event: '*',
        schema: 'public',
        table: 'confec_kanban_card_comments',
        filter: `card_id=eq.${cardId}`,
      }, () => {
        void queryClient.invalidateQueries({ queryKey: ['confec-card-comments', cardId] });
      })
      .subscribe();
  ...
  const addComment = useMutation({
    ...
    onMutate: async (content: string) => {
      await queryClient.cancelQueries({ queryKey: ['confec-card-comments', cardId] });
      const previous = queryClient.getQueryData<DevCommentRow[]>(['confec-card-comments', cardId]);
      ...
      queryClient.setQueryData<DevCommentRow[]>(['confec-card-comments', cardId], (old = []) => [
        optimistic,
        ...old,
      ]);
      setText('');
    ...
    onError: (_err, _content, ctx) => {
      if (ctx?.previous) {
        queryClient.setQueryData(['confec-card-comments', cardId], ctx.previous);
      }
    ...
    onSuccess: (result) => {
      ...
      void (async () => {
        try {
          ...
          console.warn('[ConfecCardComments] notify failed:', e);
        }
      })();
    },
  });
  ...
  const deleteComment = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await supabase.from('confec_kanban_card_comments').delete().eq('id', id);
      if (error) throw error;
      return id;
    },
    onSuccess: (id) => {
