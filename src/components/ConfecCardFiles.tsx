...
interface ConfecCardFilesProps {
  cardId: string;
}
...
export function ConfecCardFiles({ cardId }: ConfecCardFilesProps) {
  const { user } = useAuth();
  ...
  const { data: files = [], isLoading, isError, isFetched, refetch } = useQuery({
    queryKey: ['confec-card-files', cardId],
    queryFn: async () =>
      runTimedQuery(async () => {
        const { data, error } = await supabase
          .from('confec_kanban_card_files')
          .select('*')
          .eq('card_id', cardId)
  ...
  const handleDelete = async () => {
    ...
    if (!file) return;
    try {
      await supabase.storage.from('confec-kanban-files').remove([file.file_path]);
      const { error } = await supabase.from('confec_kanban_card_files').delete().eq('id', deleteId);
      if (error) throw error;
