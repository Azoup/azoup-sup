...
export function DevCardFiles({ cardId }: DevCardFilesProps) {
  const { user } = useAuth();
  ...
  const handleDelete = async () => {
    ...
    if (!file) return;
    try {
      await supabase.storage.from('dev-kanban-files').remove([file.file_path]);
      const { error } = await supabase.from('dev_kanban_card_files').delete().eq('id', deleteId);
      if (error) throw error;
