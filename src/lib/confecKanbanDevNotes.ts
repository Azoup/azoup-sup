...
export function isConfecNotesSchemaError(message: string | undefined): boolean {
  if (!message) return false;
  const m = message.toLowerCase();
  return m.includes('dev_notes') && (m.includes('schema cache') || m.includes('column'));
}

export async function loadConfecKanbanNotes(
  cardId: string,
  ...
  if (cardErr && !isConfecNotesSchemaError(cardErr.message)) throw cardErr;
...
async function persistDevNotesCommentFallback(
  ...
  if (!trimmed) {
    if (existing?.id) {
      const { error } = await supabase.from('confec_kanban_card_comments').delete().eq('id', existing.id);
      if (error) throw error;
    }
    return;
  }
  ...
  const { error } = await supabase.from('confec_kanban_card_comments').insert({
    card_id: cardId,
    user_id: userId,
...
export async function saveConfecKanbanNotes(
  cardId: string,
  notes: string | null,
  userId: string,
  userEmail: string,
  ...
  if (!isConfecNotesSchemaError(error.message)) throw error;
