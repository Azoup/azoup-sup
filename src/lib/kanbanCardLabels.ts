...
export type CardLabelTable = 'kanban_card_labels' | 'dev_kanban_card_labels' | 'confec_kanban_card_labels';
...
export function labelsForCardFromRows(
  rows: {
    label_id: string;
    kanban_labels?: unknown;
    dev_kanban_labels?: unknown;
    confec_kanban_labels?: unknown;
  }[],
): { id: string; name?: string; color?: string }[] {
  const byId = new Map<string, { id: string; name?: string; color?: string }>();
  for (const row of rows) {
    const nested = (row.kanban_labels ?? row.dev_kanban_labels ?? row.confec_kanban_labels) as
      | { id?: string; name?: string; color?: string }
      | null
      | undefined;
