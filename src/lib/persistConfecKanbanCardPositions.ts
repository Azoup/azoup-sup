...
async function updateCardPosition(
  ...
  let { error } = await supabase.from('confec_kanban_cards').update(payload).eq('id', id);
  if (error && `${error.message}`.toLowerCase().includes('completed_at')) {
    const retry = await supabase.from('confec_kanban_cards').update({ status, position }).eq('id', id);
    error = retry.error;
  }
...
export async function persistConfecKanbanCardPositions(
