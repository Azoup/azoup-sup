...
import { useConfecKanbanBoard, refreshConfecKanbanBoard, flushConfecKanbanBoardRefresh } from '@/hooks/useConfecKanbanBoard';
import { clearConfecKanbanBoardCache } from '@/lib/confecKanbanBoardCache';
import { isSupabaseLockError, withSupabaseRetry } from '@/lib/supabaseRetry';
import {
  patchConfecKanbanBoardCards,
  patchConfecKanbanBoardCardLabels,
  patchConfecKanbanBoardCardImages,
  patchConfecKanbanBoardColumns,
  CONFEC_KANBAN_BOARD_QUERY_KEY,
} from '@/lib/confecKanbanBoardPatch';
...
function ConfecKanbanCardFooter({
  card,
  completedAt,
}: {
  card: {
...
const KanbanConfec = () => {
  ...
  const { data: board, isLoading: boardLoading } = useConfecKanbanBoard(supabaseReady);
  ...
  const checklistProgress = useChecklistProgressMap('confec', supabaseReady && !!board);
  ...
  useEffect(() => {
    ...
    if (!hasTicketNumber) {
      ...
      clearConfecKanbanBoardCache();
      flushConfecKanbanBoardRefresh(queryClient);
    }
  ...
  const cardsByColumn = useMemo(() => {
    ...
    cards.forEach((card: any) => {
      ...
      const enriched = {
        ...card,
        labels: labelsForCardFromRows(cls as { label_id: string; confec_kanban_labels?: unknown }[]),
        analyst,
        developer,
        images,
      };
  ...
  const runPostSaveBackground = useCallback(
    async (opts: {
      ...
      if (imageFiles.length > 0) {
        ...
        if (images.length > 0) {
          patchConfecKanbanBoardCardImages(queryClient, (rows) => [...rows, ...images]);
        }
        if (failed > 0) {
          toast.error(
      ...
      if (attachmentFiles.length > 0) {
        ...
        queryClient.invalidateQueries({ queryKey: ['confec-card-files', cardId] });
      }
      ...
      if (notify && (developerId || analystId)) {
        try {
          ...
          console.warn('[kanban-confec] notificação:', notifyErr);
        }
      }
    },
  ...
  const createCard = useMutation({
      ...
      return {
        ...
        notify:
          developerId || analystId
            ...
            : undefined,
        logLabel: 'Criou card no Kanban Confec',
    ...
    onSuccess: (result) => {
      ...
      if (newCard) {
        ...
        patchConfecKanbanBoardCards(queryClient, (list) => [...list, newCard]);
        patchConfecKanbanBoardCardLabels(queryClient, (rows) => {
          ...
          const next = selectedLabels.map((label_id) => {
            ...
            return {
              card_id: newCard.id,
              label_id,
              confec_kanban_labels: label ?? { id: label_id },
            };
          });
          return [...rest, ...next];
        });
  ...
  const updateCard = useMutation({
    mutationFn: async () => {
      ...
      await syncCardLabels('confec_kanban_card_labels', editingCard.id, selectedLabels);
      ...
      return {
        ...
        attachmentFiles,
        notify,
        logLabel: 'Editou card no Kanban Confec',
        logDetails,
  ...
  const deleteCard = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await supabase.from('confec_kanban_cards').delete().eq('id', id);
      if (error) throw error;
      await logActivity('Excluiu card do Kanban Confec');
    },
    onSuccess: () => {
  ...
  const deleteImage = useMutation({
    mutationFn: async (imageId: string) => {
      const { error } = await supabase.from('confec_kanban_card_images').delete().eq('id', imageId);
      if (error) throw error;
      return imageId;
    },
    onSuccess: (imageId) => {
      markBoardLocalWrite(2);
      patchConfecKanbanBoardCardImages(queryClient, (rows) =>
  ...
  const createLabel = useMutation({
    mutationFn: async () => {
      const { error } = await supabase.from('confec_kanban_labels').insert({ name: newLabelName, color: newLabelColor });
      if (error) throw error;
    },
    onSuccess: () => {
      refreshConfecKanbanBoard(queryClient);
  ...
  const updateLabel = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await supabase.from('confec_kanban_labels')
  ...
  const deleteLabel = useMutation({
    mutationFn: async (id: string) => {
      await supabase.from('confec_kanban_card_labels').delete().eq('label_id', id);
      const { error } = await supabase.from('confec_kanban_labels').delete().eq('id', id);
      if (error) throw error;
    },
    onSuccess: () => {
      refreshConfecKanbanBoard(queryClient);
  ...
  const addColumn = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await supabase.from('confec_kanban_columns').insert({
  ...
  const editColumn = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await supabase.from('confec_kanban_columns')
  ...
  const removeColumn = useMutation({
    mutationFn: async (id: string) => {
      ...
      if (firstCol) {
        await supabase.from('confec_kanban_cards').update({ status: firstCol.slug }).eq('status', col.slug);
      }
      const { error } = await supabase.from('confec_kanban_columns').delete().eq('id', id);
      if (error) throw error;
    },
    onSuccess: () => {
      refreshConfecKanbanBoard(queryClient);
  ...
  const ensureDoneLabel = useCallback(async (): Promise<string | null> => {
    ...
    const { data, error } = await supabase.from('confec_kanban_labels')
      .insert({ name: 'Concluído', color: '#10b981' })
      .select().single();
    if (error || !data) {
      const fallback = await supabase
        .from('confec_kanban_labels')
        .select('id,name')
        .ilike('name', '%conclu%')
        .limit(1)
        .maybeSingle();
  ...
  const applyDoneLabelRule = useCallback(async (cardId: string, fromSlug: string, toSlug: string) => {
    ...
    if (movedToDone) {
      ...
      await syncCardLabels('confec_kanban_card_labels', cardId, [doneLabelId]);
      patchConfecKanbanBoardCardLabels(queryClient, (old) => {
        const targetLabel = (labels as any[]).find((l) => l.id === doneLabelId);
        return [
          ...old.filter((cl: any) => cl.card_id !== cardId),
          { card_id: cardId, label_id: doneLabelId, confec_kanban_labels: targetLabel },
        ];
      });
      ...
      await syncCardLabels('confec_kanban_card_labels', cardId, remaining);
      patchConfecKanbanBoardCardLabels(queryClient, (old) =>
  ...
  const onDragEnd = useCallback((result: DropResult) => {
    ...
    patchConfecKanbanBoardCards(queryClient, (old) =>
      old.map((card: any) => {
    ...
    void (async () => {
      try {
        await withSupabaseRetry(() =>
          Promise.all([
            persistConfecKanbanCardPositions(boardCards, positionUpdates, {
              draggableId,
              completedAtOnMove,
            }),
            statusChanged
        ...
        if (previousCards) {
          patchConfecKanbanBoardCards(queryClient, () => previousCards);
        } else {
          flushConfecKanbanBoardRefresh(queryClient);
        }
  ...
  const openEdit = (card: any) => {
    ...
    setPendingFiles([]);
    setEditOpen(true);
    void loadConfecKanbanNotes(card.id, card.dev_notes).then((notes) => {
      setDevNotes(notes);
  ...
  const openView = (card: any) => {
    ...
    setViewingDevNotes('');
    setViewOpen(true);
    void loadConfecKanbanNotes(card.id, card.dev_notes).then(setViewingDevNotes).catch(() => {});
  };
  ...
  useEffect(() => {
    ...
    if (target) {
      ...
      setViewingDevNotes('');
      setViewOpen(true);
      void loadConfecKanbanNotes(target.id, target.dev_notes).then(setViewingDevNotes).catch(() => {});
  ...
  const moveColumn = useCallback(async (colId: string, direction: 'left' | 'right') => {
    ...
    patchConfecKanbanBoardColumns(queryClient, (old) =>
      old.map((c: any) =>
        c.id === a.id ? { ...c, position: b.position } : c.id === b.id ? { ...c, position: a.position } : c,
      ),
    );
    await Promise.all([
      supabase.from('confec_kanban_columns').update({ position: b.position }).eq('id', a.id),
      supabase.from('confec_kanban_columns').update({ position: a.position }).eq('id', b.id),
    ]);
    refreshConfecKanbanBoard(queryClient);
  ...
  return (
    <div className="flex h-[calc(100dvh-5.5rem)] max-h-[calc(100dvh-5.5rem)] min-h-0 flex-col gap-3 overflow-hidden animate-fade-in md:h-[calc(100dvh-6.5rem)] md:max-h-[calc(100dvh-6.5rem)]">
          ...
          {viewingCard && (
            <>
              <div className="min-h-0 min-w-0 flex-1 space-y-4 overflow-y-auto overflow-x-hidden overscroll-y-contain px-6 pb-4 [-webkit-overflow-scrolling:touch]">
                ...
                <CardChecklist cardId={viewingCard.id} cardType="confec" description={viewingCard.description} />
                <ConfecCardFiles cardId={viewingCard.id} />
                <ConfecCardComments cardId={viewingCard.id} />
              </div>
      ...
      <Dialog open={addColumnOpen} onOpenChange={setAddColumnOpen}>
        <DialogContent className="max-w-sm">
          <DialogHeader><DialogTitle>Nova Lista</DialogTitle><DialogDescription>Crie uma nova coluna no Kanban Confec.</DialogDescription></DialogHeader>
          <div className="space-y-3">
...
function ConfecCardFormContent({
